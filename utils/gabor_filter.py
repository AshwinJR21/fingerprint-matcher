...
def gabor_filter(im, orient, freq, kx=0.65, ky=0.65):
    ...
    # frequency_ind = np.where(freq > 0)
    # if len(frequency_ind[0]) == 0:
    #     raise ValueError("No non-zero frequency values found in the freq array.")
    non_zero_elems_in_freq = freq_1d[frequency_ind]
    non_zero_elems_in_freq = np.double(np.round((non_zero_elems_in_freq*100)))/100
    unfreq = np.unique(non_zero_elems_in_freq)

    if unfreq.size == 0:
        raise ValueError("Error: 'unfreq' is empty. Ensure non_zero_elems_in_freq has valid data.")
