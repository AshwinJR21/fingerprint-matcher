...
def fingerprint_pipline(input_img):
    ...
    for i in range(len(output_imgs)):
        ...
        cv.imshow("output", output_imgs[i])
        cv.waitKey(0)
    ...
    #return results
img = cv.imread(r"D:\amrita\sem3\introduction to AI and ML\datasets\dataset_fingerprint\SOCOFing\Real\1__M_Right_index_finger.BMP")
img = cv.cvtColor(img, cv.COLOR_BGR2GRAY)   
img = cv.resize(img, (500, 500))
fingerprint_pipline(img)

# if __name__ == '__main__':
#     # open images
#     img_dir = r'D:\amrita\sem3\introduction to AI and ML\datasets\dataset_fingerprint\sample_inputs\*'
#     output_dir = './output/'
