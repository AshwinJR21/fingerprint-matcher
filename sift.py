...
def sift_process(image_file):
    ...
    te = time.time()
    print('time taken for sift:', te - ts)
    ...
    # Send the processed image back to the frontend(return statement)

    print(r"D:\amrita\sem3\introduction to AI and ML\datasets\dataset_fingerprint\SOCOFing\Real\\"+filename)
    index = np.where(data['image path'] == rf"D:\amrita\sem3\introduction to AI and ML\datasets\dataset_fingerprint\SOCOFing\Real\{filename}")[0][0]
    row = data.iloc[index, :]
